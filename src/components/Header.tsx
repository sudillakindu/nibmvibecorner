...
export const Header = ({
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        ...
        <nav className="hidden md:flex items-center space-x-6">
          {navItems.map(item => (
            <a
              ...
              className={`group relative flex flex-col items-center transition-colors ${
                activeLink === item.id 
                  ? isDarkMode 
                    ? 'text-mustard-500' 
                    : scrolled 
            ...
            >
              ...
              <span className={`absolute -bottom-1 left-0 w-full h-0.5 transform scale-x-0 transition-transform origin-left group-hover:scale-x-100 ${
                activeLink === item.id 
      ...
      <div className={`md:hidden overflow-hidden transition-all duration-500 ease-in-out mr-3 ml-3 ${
            ...
            <div className="flex flex-col space-y-2">
              {navItems.map((item, index) => (
                <a
                  ...
                  href={`#${item.id}`}
                  onClick={handleNavClick(item.id)}
                  className={`flex items-center py-3 px-4 rounded-lg transition-all duration-500 transform hover:scale-105 hover:shadow-md ${
                    activeLink === item.id 
                      ? isDarkMode 
